...
Current workstream: **Stripe billing — Phase 4 (Customer Portal) SHIPPED (unverified) — CLICK-TEST NEXT; Phases 0-3 DONE**
...
## Session log (2026-06-10) — Stripe Phase 4 (Customer Portal) SHIPPED, awaiting click-test
- Phase 4 built (LORAMER_STRIPE_PHASE4_PORTAL_V1). New POST /api/billing/portal (auth; fails 500 config_missing if STRIPE_PORTAL_CONFIG_ID unset; 403 manual_tier; requires a this-livemode active/trialing/past_due sub else 409 no_subscription; customer id from user_profiles w/ subscriptions backstop else 409 no_customer; creates billingPortal session w/ configuration=env + return_url=/billing). /billing hasActiveSub placeholder replaced with a real "Manage billing" button → portal. webhook/schema unchanged — portal switch/cancel ride the existing customer.subscription.updated/deleted handlers (grace via cancel at_period_end; manual-tier + out-of-order guards intact).
- DASHBOARD ACTIVATION DEFERRED: Stripe Dashboard portal setup forces full business verification (held to Phase 6/LIVE). Instead the TEST portal config was created via API: scripts/stripe-create-portal-config.mjs → **bpc_1Tgo2JEAFDrT56pML9lyrATD** (livemode=false). Config: subscription_update enabled (Business/Agency/Scale, both prices each, default proration), subscription_cancel at_period_end, payment_method_update enabled, no quantity updates. Pinned in Vercel **Production env STRIPE_PORTAL_CONFIG_ID** (non-secret id).
- ⚠️ PHASE 6 MUST repeat config creation in LIVE mode (portal configs do NOT cross modes) and set the LIVE STRIPE_PORTAL_CONFIG_ID. loramer.com/privacy + /terms both 200 (used in business_profile).
- tsc clean. NOT YET VERIFIED — click-test (plan switch + cancel sync to Supabase) pending; staged revert = `git revert HEAD --no-edit && git push` (code-only; leaving the Stripe config is harmless).
...
## NEXT STEP — Verify Stripe Phase 4 (Customer Portal) end-to-end in TEST on prod. Sign in as a test user, subscribe (4242) → /billing shows "Manage billing" → portal opens (config bpc_1Tgo2JEAFDrT56pML9lyrATD). Prove BOTH sync paths via Supabase MCP: (A) plan switch Business→Agency → customer.subscription.updated → subscriptions.tier + user_profiles.tier = agency; (B) cancel at period end → subscriptions.cancel_at_period_end=true, status active, tier unchanged (grace); then immediate cancel (`stripe subscriptions cancel <id> --confirm`) → customer.subscription.deleted → tier=free. Negative: free user POST /api/billing/portal → 409; button absent for free + manual tiers. Then sweep test data to 0 + delete Stripe test customer. Staged revert ready (`git revert HEAD --no-edit && git push`). After Phase 4 verified → Phase 5 (Gating). Also queued: enable Google Pay in Stripe payment settings (roadmap). External clocks: Google OAuth adwords scope UNDER REVIEW; Meta access verification DONE (Tech Provider cleared 2026-06-09) — Meta App Review for ads_read (then switch app to Live mode) is the next ACTIVE workstream alongside the Google OAuth review; cohort can't connect Meta until both clear (see ROADMAP Pre-launch requirements).
