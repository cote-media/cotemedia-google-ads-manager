...
- WS2 #2b — CONNECTION-STATE PROBE PARITY AUDIT (read-only): apply the Lesson-60 standard to Shopify, GA, WooCommerce. For each: does ANY single error flip durable platform_connections.health to 'reconnect'/unhealthy WITHOUT a real live/refresh probe, and can it fan out across connections? Expected (per WS2 #2 investigation): their 'broken' signal already comes from a real token-refresh attempt (getValidShopifyToken / getValidGaToken auto-refresh = probe-by-nature) and scope is per-connection, not fleet — so they're likely already evidence-backed. DELIVERABLE: prove it (document why each is safe) OR, if any infers death from a single un-probed error, apply the same probe-before-flip fix. Also covers the GA token-unavailable DIRECT recordConnectionAuthFailure at cron/sync:616 (bypasses recordConnectionResult — confirm it can't false-flag). Read-only first; fix as its own change-in-flight. Makes "no platform writes broken without proof" UNIVERSAL (Lesson 60). src: WS2 #2 follow-up. open [LC]
