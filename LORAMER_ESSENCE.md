## ⛔ THE STANDARD — LORAMER_ENGINEER_OF_RECORD_V1 [READ FIRST. ABOVE THE LAW, BECAUSE IT GOVERNS HOW THE LAW IS APPLIED]
**YOU ARE THE ENGINEER OF RECORD.** This system's core failures are INVISIBLE: wrong looks exactly like right —
rows go up, jobs return 200, nothing errors, every check reads green. Seven defects on 2026-08-17 were found by
looking, none by an alarm. Therefore, on every flight:
- **NAME THE FAILURE MODE BEFORE WRITING CODE.** If you cannot say how this breaks silently, you are not ready.
- **NAME EVERY READER AND WRITER — INCLUDING THE DATABASE.** A CHECK constraint is a reader. So is a guard fixture.
- **READ PRIOR ART FIRST** — the repo's own shipped version, then the outside world. Copying beats reinventing.
- **DESIGN THE DETECTOR BEFORE THE IMPLEMENTATION.** If it cannot be seen failing, it cannot be trusted passing.
- ⛔ **A FIX THAT CLOSES A HOLE AND LEAVES THE CLASS ALIVE IS A FAILURE, EVEN SHIPPED GREEN.**
- **PREFER A GUARD TO A SENTENCE.** Prose laws here are 0-for-6; coded laws are unbroken. Measured, not felt.
⛔ **THIS APPLIES TO THE WHOLE APP** — capture, coverage, evals, the meter, Lora's grounding — not just backfill.
...
- [LAW] ⛔ THE GOOGLE LEGACY FREEZE IS LIFTED (2026-09-21; Russ 2026-09-16 and 2026-09-21; DECISIONS LORAMER_GOOGLE_ACCESS_STANDARD_V1 owns the record). The freeze of 2026-08-14/15 protected the exhibit of the Standard Access review — legacy /dashboard and the demo twin 2617b163 — and that review closed with the 2026-09-15 grant. What it was NOT and still binds: ruling (n) one writer per surface (the 52 legacy keys stay with their builders until ★LEGACY-RETIREMENT's remaining gates), the forward driver owns the 319 catalogue surfaces only and has its own caller, and the other four platforms' legacy paths are untouched by the lift. Runtime residue of the freeze (the twin's driver exclusion, the twin's held ad-name repair) is queued as ★STALE-ACCESS-CLAIMS-IN-RUNTIME-STRINGS, not silently changed. (Digest §C prints this section daily — build-resume-digest.mjs:77.)
