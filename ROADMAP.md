...
- [x] Shopify App Store review — APPROVED (no feedback pending)
...
- [ ] **Site-wide info tooltips (LORAMER_ROADMAP_INFO_TOOLTIPS_V1):** Cross-cutting UX — add ("i") explainers across the app wherever metrics or terms may confuse users. Examples: "Net sales = product revenue after refunds, excl shipping & tax"; ROAS vs CPA; impression share decomposition. Not one screen — many surfaces, one consistent tooltip pattern. → Consolidated into LORAMER_ROADMAP_TIPS_GLOSSARY_V1 (above).
  - ADDED 2026-09-09 (★NINE-THIRTY-PROOF-SCOPE — DECISIONS LORAMER_PROOF_SCOPE_9_30_V1): the RESTATEMENT-WINDOW explainer. A day inside the vendor's restatement window is labeled "held, inside the restatement window" as distinct from "final"; the ("i") says why a recent figure may still move (Google: a 90-day click-through lookback on most accounts, read daily from conversion_action) and that it becomes final automatically when the lookback lane's window expires. UX roadmap sub-item of this entry — no new token.

  ### Revenue-basis tooltips (per platform — wording is authoritative, do not paraphrase loosely)
  Net sales is computed per platform and the tooltip MUST state the platform's actual basis:
  - Shopify  → "Net sales = product revenue after refunds, excluding shipping & tax (subtotal basis)."
  - WooCommerce → "Net sales = order revenue after refunds, including shipping & tax, allocated to products pro-rata by gross share."
  - Stored truth: each product row's extra.netBasis tag self-describes the basis ('account_net_incl_shipping_tax_prorata_by_gross_share' for Woo; Shopify uses its subtotal-excl basis). Tooltips should reflect the stored basis, never assume cross-platform sameness.
  Reason: per-client consistency is exact (Σproduct ≡ account, residual 0); cross-PLATFORM basis differs (Woo incl shipping/tax vs Shopify subtotal-excl). A single client running BOTH platforms is near-impossible, so within-client numbers are always consistent; the tooltip carries the cross-client honesty.
...
**THIRD-PARTY ATTRIBUTION IS AN OVERLAY, NEVER A SOURCE (LORAMER_THIRD_PARTY_ATTRIBUTION_OVERLAY_V1, banked 2026-07-19).** Triple Whale, Northbeam, Polar, or any other re-attribution layer enters LoraMer ONLY as a LABELED PROVENANCE OVERLAY: a voice Lora can quote and explain, clearly marked "<tool> says", shown alongside the platform's own number and the store's own number. It is NEVER summed into anything, NEVER blended, and NEVER captured as a source in `metrics_daily`. This is a direct extension of the multi-source metric provenance law (ESSENCE owns that law; this is its application to re-attribution products, not a restatement of it) — the whole point is that Lora holds every source at once, labels each, and explains WHY they disagree. A re-attribution tool is one more opinion with a name on it, not a truth to fold into the record. Capturing one as a source would import somebody else's model INTO our system of record and quietly destroy the thing that makes the record worth having.
⚠ OPEN GATE BEFORE ANY SUCH OVERLAY IS BUILT: verify the specific tool's API terms of service permit a COMPETING analytics product to pull a shared client's data. Several attribution vendors restrict exactly this. This is a legal/ToS check, not an engineering one, and it gates the work — do not build the overlay and find out afterwards.
...
- [x] **THE QUEUE IS REMOVED — the cron does the walk INLINE. 2026-08-22, LORAMER_QUEUE_REMOVED_INLINE_WALK_V1.** Three legs concurred (architecture read, independent research, adversary): the trigger is a clock, the work list is DERIVED (stored work lists are guard-banned), the consumer group was one, and the queue's guarantees were each duplicated by a stronger app-level mechanism. The fire now executes its own selection unit-by-unit via `processMessage` (terminal row per unit, per-unit budget reservation, first unit always admitted); the operator drive converts in the same commit; the mis-size narrowing is a DIRECT CONTINUATION (upper half first, three bounds: halving / fire deadline / meter). OVERLAP is excluded by `universe_fire_lease` (migrations/085) — CAS in DB time, TTL 330s > the 300s ceiling, the relationship guard-pinned (`inline-fire-fits-the-ceiling.guard.mjs`); Vercel documents cron overlap, duplicate invocation AND deploy-straddle, and the lease covers all three plus drive-vs-cron. `check-consumer-liveness` was RE-FOUNDED in the same commit (EXECUTION_DARK + LEASE_WEDGED reds). ⚠ **`universe_fire_log` COLUMN SEMANTICS WIDENED BY THIS SHIP: `published` now means UNITS SELECTED FOR EXECUTION and `elapsed_ms` now spans SCAN + CAPTURE (~75-125s expected, up from ~48s scan-only) — a reader comparing across 2026-08-22 spans two regimes.** Poll lane deleted (`universe-drain-poll` route + cron); `@vercel/queue` stays for the v1 lane only. Gate-A: 37/37 units full ledger chain on a real wet fire · concurrent fire refused in 0.28s naming the holder · zero new day_committed duplicates across two fires · dry run executes nothing and takes no lease.
- [x] **Walk delivery moved from PUSH to CRON-DRIVEN POLL — 2026-08-22, LORAMER_POLL_MODE_CUTOVER_V1 (`fbe1c4b`).** The Vercel Queues push consumer decayed to zero every 3–6h and was restored ONLY by a deploy (five dark spans in 41h, five deploy-restores, measured from `universe_attempt_log`). The cron path never missed a fire across the same window, so delivery moved onto it: `/api/cron/universe-drain-poll` polls the topic on the producer’s cadence and the `experimentalTriggers` entry was removed in the same commit. Exactly one lane per topic, enforced by `one-delivery-lane-per-topic.guard.mjs` (red on two lanes AND on zero). ⛔ **THE PUSH DECAY IS ROUTED AROUND, NOT DIAGNOSED** — ★DELIVERY-DECAY-CAUSE-UNKNOWN stays open. ⛔ **THE BACKLOG-DRAIN LANE IS NOT SHIPPED** and is blocked on a consumer-side op gate (★DRAIN-LANE-BLOCKED-NO-CONSUMER-SIDE-OP-GATE): the daily Google op cap is enforced at publish time only, and a replay does not publish.
- [ ] **MONDAY TARGET — one-click forward + backward, existing client AND cold connect. FOUR PARTS, THREE UNMEASURED (★MONDAY-GAP-LIST-IS-UNMEASURED, 2026-08-22).** (1) DELIVERY — routed around by the poll cutover above; NOT proven past 3h and the push decay’s cause is still unknown. (2) **META / GA4 / SHOPIFY / WOO ARE UNPORTED** — only Google Ads runs on the universe walk. (3) **THE COLD-CONNECT PATH HAS NEVER BEEN TESTED END TO END** — LORAMER_BACKFILL_DONE_DONE_V1 requires a genuinely cold connection through the real entry path and that run has never happened. (4) **FORWARD + BACKWARD ON ONE CLICK HAS NEVER BEEN PROVEN TOGETHER.** ⛔ No number in this repo says how far from done 2–4 are; scoping them is the first task of the next session, before any build.
...
- [x] **Google Ads API Standard Access — GRANTED 2026-09-15 (DECISIONS LORAMER_GOOGLE_ACCESS_STANDARD_V1 owns the status; the plan below is history).** Apply in the Ads MCC API Center (tied to the developer token, NOT the GCP project); declare **External + Reporting-Only** permissible use (code is 100% read-only — only `customer.query`/`googleAds:search`, zero mutates). RMF does NOT require write features for a reporting-only tool — only required default columns + a clearly-labeled export per displayed hierarchy level. **Basic (15k ops/day) is almost certainly enough for the invite-only July 14 cohort — Standard is a scale-time item, NOT a launch blocker** (and Google has a known 2026 review backlog). Approach drafted 2026-06-10 (see CONTINUE_HERE / session report); no submission yet.
- Meta App Review — ✅ DONE (APPROVED + PUBLISHED 2026-07-02; added 2026-06-10). `ads_read` granted, app Live; the founding cohort can connect Meta. Historical: submitted a read-only justification + screencast; ran alongside the now-CLOSED Google OAuth verification. No active review remains.
...
- [ ] Lora Honesty Spec → system prompt (2026-06-11, part of 95% accuracy path): encode the CLAIM-CONFIDENCE RULE (LORAMER_DECISIONS.md → Claim-Confidence Rule / LORAMER_CLAIM_CONFIDENCE_RULE_V1) as Lora's in-product honesty doctrine at launch — the no-ceremony three-level confidence gate (VERIFIED = read/queried this turn · DERIVED = reasoning shown · UNVERIFIED = memory/pattern → query-don't-guess or say "haven't verified, want me to check?" and stop), gaps-out-loud, corrections-outrank-memory, no unverified meta-claims ("nothing has changed"), no speculation tails (e.g. "resolve billing issues" absent evidence). Obey-don't-narrate (NO typed VERIFIED/DERIVED/UNVERIFIED labels). Pairs with the provenance footer.
...
- [ ] **Structured-accuracy program (the 21%→95% stack)** — per Anthropic's self-service-analytics post (claude.com/blog/how-anthropic-enables-self-service-data-analytics-with-claude): (a) finish `query_metrics` and make it Lora's MANDATORY first path with pre-rebutted fallback excuses; (b) pairwise Skills — thin router + per-platform reference docs (grain, scope, gotchas, date/timezone conventions) following the post's skeleton; (c) maintenance rule: any adapter/schema change updates the matching skill doc in the SAME commit (their accuracy decayed 95→65 in one month without this); (d) offline eval suite — a few dozen pinned-snapshot Q/A pairs per platform, results stored as telemetry, ~90% per-domain gate before cohort exposure; (e) provenance footer on every Lora answer (source tier + data freshness); (f) correction harvesting — user corrections become eval candidates + doc fixes; track %-via-`query_metrics` and correction rate weekly. NOTE the post's null result: raw retrieval over historical queries/chats moved accuracy <1 point — distill into docs, don't dump corpus. This is the GATE item #2 above depends on. (g) MODEL-AGNOSTIC EVAL HARNESS (extends (d), 2026-06-25): frozen (question, dataset-snapshot) pairs with ground truth COMPUTED from the frozen data (not memorized), run against ANY model to prove Lora's correctness is model-STABLE (defends against model-version drift); only computable-answer questions (top combination by conversions, highest-ROAS, spend-by-device) — judgment questions assessed separately; anchor ground truth to the snapshot so it can't drift (per the Anthropic self-serve analytics blog). One artifact serves accuracy + version-independence. (h) PLATFORM API REFERENCE SET (extends (b)/(c), 2026-06-25): per-platform repo docs — error taxonomy (retry/fix/escalate), rate limits/quotas, retention floors, pagination, gotchas, API version — each stamped with source URL + date-captured + API-version. BIND TO BUILDS: any plan/build/push touching a platform's API consults its reference file FIRST and re-verifies correctness-critical facts against live docs before relying (snapshot = fast path, live re-verify = gate); re-verify on cadence + whenever a platform error surprises us. Seeded by the 2026-06-25 VERIFIED Meta error taxonomy ({1,2,4,17,613,80004}=retry · 100·1487534=narrow-window · 190·368=surface). Same staleness discipline as colocated skill docs (95%→65% without maintenance).
...
## LORA WEB RESEARCH — capability + scoping principle (queued; enhancement, post-launch-friendly; elaborates the "external web research" edge in LORAMER_LORA_INTELLIGENCE_BAR)
WHY: Lora currently says she can't search the web — most likely web_search is simply not wired into her tool set yet (NOT an intentional disable). Motivating use: correlate a client's performance with industry-wide platform changes, backed by external evidence — e.g. the "Andromeda thesis": Foam OH's Meta dip vs Meta's Andromeda ad-retrieval rollout; search other businesses' news/blogs on how Andromeda affected their ads. This IS the cross-platform + external-research analysis that defines the intelligence bar.
...
- [ ] **PRICING FOR LARGE BACKFILLS (2026-06-25):** deep historical fills (years × many grains × cohort) carry real API/compute cost at scale, AMPLIFIED by auto-backfill-on-connect (LORAMER_ONBOARD_DRAIN_V1) draining every NEW connection to floor. Design a pricing lever: backfill depth/volume gated by tier and/or a one-time deep-fill charge per connection. Scope BEFORE mass onboarding (~1,000 users × ~5 connections target). Cross-ref Project 2 (Pricing & Billing) + the drain N-map throughput knob.
...
## 🔌 MCP HOST / AI-READY DATA ACCESS TIER — banked 2026-07-19 (post-launch)
<!-- LORAMER_MCP_HOST_TIER_V1 -->
- [ ] **MCP HOST / AI-READY DATA ACCESS (post-launch):** expose the 6-layer intelligence via an AUTHED MCP server so enterprise clients query LoraMer from their OWN AI clients (Claude Desktop, their internal agents) instead of only through our UI. The product claim is the same one the whole capture program earns — full-grain history nobody else holds — delivered where the customer already works. **GATED ON: usage metering + caps existing** (below). Without metering there is no way to price, cap, or even see what an external agent consumes, and an MCP seat is an uncapped read loop by construction. Cross-ref: `mcp-server.js` already exposes Google Ads tools to Claude Desktop as a LOCAL, single-user, unauthed dev tool — that is the shape, not the product; the tier needs auth, tenancy (the ACCESS-LAYER LAW: reads run on the client's real owner), and metering before it is sellable.

- [ ] **USAGE METERING + CAPS (shared prerequisite, launch-relevant TODAY for cost, revenue-blocking LATER):** per-org / per-member metering of Anthropic spend + query volume, with enforceable caps. TWO jobs, one mechanism:
  1. **COST EXPOSURE, live now.** There is currently NO entitlement enforcement anywhere outside the Stripe webhook — no questions-per-month check, no per-member cap, nothing gating `/api/chat` or any `/api/next/*` route. Every cohort member gets unlimited Opus against our account, and Lora BLANKS at a $0 balance (the known credit-depletion failure mode). Cost scales with engagement, so the best customers cost the most — the COST LAW already banked against per-org memory applies here too.
  2. **REVENUE, later.** The same meter is what makes the MCP tier billable. It GRADUATES from "cost hygiene" to "revenue-blocking" the moment that tier is real.
  Entitlement fields already exist unused in the billing UI (`questions_per_month`, `history_window_days`, `workspace_cap` in `src/lib/billing/plans.ts`) — the gap is enforcement + measurement, not schema design.

OPEN, SEPARATE, NOT PART OF THE ABOVE — **daily-row UTC day-boundary vs a client-timezone read model.** Shopify order timestamps are now captured RAW to the second (`breakdown_type='order_time'`, LORAMER_SHOPIFY_ORDER_TIME_V1) precisely so history can be re-bucketed under any later timezone model with ZERO recapture. But the DAILY row's day boundary is still UTC (`created_at` window, shopify-intelligence.ts) — so hour-level and day-level answers will disagree at the edges until a client-timezone read model lands. That change is NON-ADDITIVE (it moves revenue between days) and is its own gated step — deliberately NOT bundled here. Open/closed status is owned by LORAMER_QUEUE_OF_RECORD.md; design context in the DIGEST-WINDOW-MODEL entry (timezone is a property of the CLIENT, derivable from Google `customer_client.time_zone` / Shopify `shop.ianaTimezone`, never askable-first).
