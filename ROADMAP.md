...
- [ ] **Coming-soon landing page at loramer.com (lives in SEPARATE repo `cote-media/loramer-landing`):** V1+V2 shipped May 29, 2026 to a Vercel preview URL. Next.js 14 single-page, center hero with phrase-by-phrase reveal, 4 differentiators including "A real human, always", pricing section with 5 tiers across two tracks (Free/Business $79 + the agency lane: Agency $199 / Scale $999 / Enterprise). Mailchimp waitlist working end-to-end (PUT-upsert for idempotency, PII-free env vars in Vercel only). REMAINING: point loramer.com at Vercel after Cloudflare DNS propagation completes; set up Cloudflare Email Routing for hello@loramer.com; rotate Mailchimp API key. (LORAMER_LANDING_V1 + V2)
