...
- [~] **Phase 0b query layer BUILT (June 4, 2026 - LORAMER_QUERY_METRICS_0B_V1):** src/lib/metrics-query.ts (paginated aggregation over metrics_daily + equal-length multi-period windows + derived CTR/CPC/CPA/ROAS/AOV) and read-only proving route /api/query-metrics (CRON_SECRET auth). NOT yet validated against the store and NOT yet wired into Claude chat (chat is still single-shot; the tool-use loop is the next, separate ship). Next: curl the 7d vs 6/12/18mo marquee on My Vacation Network and reconcile the numbers.
