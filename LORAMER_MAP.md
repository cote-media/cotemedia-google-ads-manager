...
- **A fire plans only what it can execute** (round 34, 2026-09-23; DECISIONS LORAMER_FIRE_PLANS_UNTIL_FULL_V1): a candidate's coverage is one server-side per-day probe (`universe_coverage_dates`, migration 103), the scan stops deriving when the planned reserve across the 12 execution lanes fills the fire budget, only executed units feed the run's no-progress clock, and a run's claim is exclusive (a lease-held fire ends the chain). Found when Tri-Copy's first pressed run starved on 360-day windows; the window stayed.
- **An instrument asks the engine which row is the account row** (round 38, 2026-09-23; DECISIONS LORAMER_ENGINE_KEYED_INSTRUMENTS_V1): googleAccountKeyFor(engine) — legacy account/'' · walk customer/customer — in a leaf module with a .mjs mirror; check:data's capture-landing judges a walk-marked connection by its customer row, at or above the customer surface's frontier while it descends and every day once sealed; the proof's instruments are guarded by name to use the predicate. The page's readiness line, the serving routes and Lora's query layer still read the legacy key — held for WIRE (★SCREEN-READERS-HELD-FOR-WIRE).
- **No push without Russ's go, fixes included; live damage while he is unreachable is a reversible hold, never a push** (2026-09-24; DECISIONS LORAMER_NO_PUSH_WITHOUT_GO_V1 — two violations recorded).
- **The engine is frozen until Tri-Copy's proof passes; an ask or skip change is proven on a whole run of a deep, empty account first; nothing is sealed empty without Google's own answer for that surface** (2026-09-24; DECISIONS LORAMER_ENGINE_FREEZE_UNTIL_PROOF_V1).
- **Alerts go by email through Resend to cotebrandmarketing@gmail.com** (2026-09-24; DECISIONS LORAMER_SESSION_2026_09_24_RULINGS_V1).
- **A Range page without a total order is not a page** (round 53, 2026-09-24; DECISIONS LORAMER_STABLE_PAGE_ORDER_V1 — rest-all keys every read; the seven unordered `.range` loops in src wait for WIRE).
- **A false that arrives absent is still a value (proto3 implicit presence), and a surface-window is re-asked by the FIRE through a queue, never by a script that asks** (round 50, 2026-09-24; DECISIONS LORAMER_IMPLICIT_PRESENCE_REASK_V1).
- **A wrong attestation is retracted by an appended row, never an update; one view (universe_attesting_terminals) owns what attests** (round 45, 2026-09-24; DECISIONS LORAMER_IDLE_SEED_RETRACTION_V1 — the false idles came from a 92-day cap on the stored seed, not a silent probe).
