...
- **A push report is not finished at the push — LORAMER_DEPLOY_POLL_UNTIL_TERMINAL_V1 (banked 2026-08-25 after two same-day stalls).** After any push to `main`, the report is INCOMPLETE until the Vercel deploy state is TERMINAL (READY or FAILED) at report time, confirmed by polling `list_deployments` against the pushed SHA (~2-min re-polls, in-turn). ⛔ **THE DEFECT THIS BANS: handing the wait to a background timer and ending the turn** — the deploy goes READY in minutes and the report arrives only when Russ prompts (45 min lost on `02e79b7`, ~20 more the same day). The wait was never the problem; stopping was. ⛔ ENFORCEABILITY, honestly: no repo guard can observe chat conduct or a Vercel poll, and no VERCEL token exists for a headless poller — `tests/guards/deploy-poll-terminal.guard.mjs` holds this rule's PLACEMENT here (the one-block-output posture); obedience's only enforcer is Russ seeing a non-terminal deploy line.
