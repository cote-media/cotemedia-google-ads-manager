...
const GUARDS = [
  ...
  'tests/guards/one-engine-writers.guard.mjs', // LORAMER_ONE_ENGINE_V1 — every old-engine writer asks legacyEngineServes before its first claim or write
  ...
  'tests/guards/fire-bite-fits-the-budget.guard.mjs', // LORAMER_FIRE_BITE_FITS_THE_BUDGET_V1 — the per-fire bite fits FIRE_WORK_BUDGET_MS at the measured worst unit cycle and carries NO scan constant; the admission rule refuses a unit that would not fit
  'tests/guards/continuous-run.guard.mjs', // LORAMER_CONTINUOUS_RUN_V1 — a backfill run chains step-to-step with no cron wait; consecutive no-progress steps END it (the poison loop a chain makes fast); only the floor ends it as done; the run path names no platform; daily upkeep stays scheduled
  'tests/guards/run-progress-signal.guard.mjs', // LORAMER_RUN_PROGRESS_SIGNAL_V1 — the continuous run reads progress under the attempt ledger's own vendor spelling (ledgerVendorFor), counts DAYS NO LONGER OWED (committed or attested by an attesting lane; never lookback, never top-edge; unreadable throws), and reports a hold only when the fire says held — never its meter line; driven on the real coverage reader with a fake ledger
  'tests/guards/run-pump.guard.mjs', // LORAMER_RUN_PUMP_V1 — the run is stepped by a cron-triggered pump INSIDE one invocation (10+ steps, zero self-requests, stops inside its deadline reserve, exits on a lost CAS, carries the shipped stop limits and fatals out verbatim); the pump is scheduled every minute, secret-gated, quiet with no active run; the run route never kicks itself (Vercel 508 at the fourth hop)
  'tests/guards/no-http-to-self.guard.mjs', // LORAMER_NO_HTTP_TO_SELF_V1 — no scheduled/background path fetches this deployment's own URL (a cron's origin is the *.vercel.app URL, which Vercel Authentication answers with a login page, not an error; a nested self-request is refused at hop 4 with 508); the run's chain is HTTP-free and the fire is the resumer's handler called in-process; remove-only allowlist with reasons
  'tests/guards/retention-wall.guard.mjs', // LORAMER_RETENTION_WALL_CANARY_V1 — an empty answer past the 37-month wall retires a day only under a SERVED canary; otherwise UNRESOLVED, loud; the fire spends nothing past the wall without a green canary
  'tests/guards/idle-skip.guard.mjs', // LORAMER_IDLE_SKIP_V1 — an idle window (Google's own account-level answer) retires across every surface for one request; a spent day is never idle; a mixed window walks whole
  'tests/guards/entity-dimension.guard.mjs', // LORAMER_ENTITY_DIMENSION_V1 — an ad names its ad group, an ad group its campaign, a campaign terminates at the account; a rename resolves to the new name; an unchanged re-read writes nothing; the dimension reads are not date-segmented reports
  'tests/guards/entity-dimension-daily-gate.guard.mjs', // LORAMER_ENTITY_DIMENSION_DAILY_GATE_V1 — the once-a-day name refresh reads the REFRESH moment (capture_pass_log 'ok' row), never the dimension's updated_at (which only moves on a change): an unchanged account is read once a day, a failed read is retried not skipped, an empty account is still a refresh, every skip leaves a pass row; driven on the real captureEntityDimension
  'tests/guards/direct-access-fallback.guard.mjs', // LORAMER_DIRECT_ACCESS_CUSTOMER_V1 — an account the customer's own login reaches directly is readable; an existing connection still makes exactly ONE manager-header attempt and is never memoised; a quota error never falls back; the stream fallback dies once a row is yielded
  'tests/guards/fire-deadline-from-fire-start.guard.mjs', // LORAMER_FIRE_DEADLINE_FROM_FIRE_START_V1 — the fire's deadline is computed ONCE from the fire's start, so no scan duration (0..400,000 ms, proven behaviourally) can push an admitted unit past the platform kill; one budget, one clock, superseded constants cannot return
  'tests/guards/fire-units-concurrent-safe.guard.mjs', // LORAMER_FIRE_UNITS_CONCURRENT_V1 — units run concurrently ONLY partitioned by surface (two on one surface can never be in flight together), the width IS the measured write-path peak and below its knee, the time-admission rule is width-independent, and refuse-and-record survives per unit
  ...
  'tests/guards/descend-window-360.guard.mjs', // LORAMER_DESCEND_WINDOW_360_V1 — maxDays 360 / MISSED_WINDOW_DAYS 360 ≤ MAX_REQUESTS_PER_RUN, pinned; deriveWindow and the missed lane never straddle the retention line (Airbyte clip form)
  'tests/guards/coverage-is-one-rpc.guard.mjs', // LORAMER_FIRE_PLANS_UNTIL_FULL_V1 — a coverage read is one RPC (per-day probes inside Postgres), ≤ 3 queries per read; migration 103 refuses > 900 days
  ...
  'tests/guards/google-delete-dialog-gate.guard.mjs', // LORAMER_GOOGLE_DELETE_DIALOG_GATE_V1 — a finished deletion is dated history on the page, never a gate on the confirm form
