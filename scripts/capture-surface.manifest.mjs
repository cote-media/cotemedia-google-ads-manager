...
export const VENDOR_SURFACE = {
  ...
  google: {
    ...
    // LORAMER_UNIVERSE_ENTITY_AXIS_V1, 2026-08-03 — the grains after 'keyword' are VENDOR-NAMED: the universe
    // walk serves segments.device from these GAQL report resources too, and under the entity axis the resource
    // IS the entity_level. The gate flagged this as a manifest UNDER-DECLARATION, which is exactly right — the
    // code captures more than the manifest called complete, and the instruction on that finding is to correct
    // the manifest, never to shrink the code. ⛔ The vendor list is derived from
    // docs/google-ads-capture-universe.json; if that artifact is re-probed, re-derive this line rather than
    // hand-adding to it (tests/guards/universe-entity-axis.guard.mjs keeps the registry half honest).
    device: { grains: ['ad', 'ad_group', 'ad_group_ad', 'ad_group_ad_asset_view', 'ad_group_asset', 'ad_group_audience_view', 'age_range_view', 'asset_field_type_view', 'asset_group', 'asset_group_asset', 'asset_group_product_group_view', 'campaign', 'campaign_asset', 'campaign_search_term_view', 'customer', 'customer_asset', 'detail_placement_view', 'expanded_landing_page_view', 'gender_view', 'geographic_view', 'group_placement_view', 'income_range_view', 'keyword', 'keyword_view', 'landing_page_view', 'paid_organic_search_term_view', 'parental_status_view', 'product_group_view', 'search_term_view', 'shopping_performance_view', 'user_location_view', 'video', 'video_enhancement'], status: 'captured', confidence: V },
    hour: { grains: ['campaign', 'ad_group', 'customer'], status: 'captured', confidence: V, note: 'ad/keyword REJECTED by Google → campaign+ad_group is complete.' },
    conversion_action: { grains: ['ad_group_asset', 'asset_field_type_view', 'asset_group_product_group_view', 'campaign', 'campaign_asset', 'campaign_budget', 'campaign_search_term_view', 'customer_asset', 'expanded_landing_page_view', 'income_range_view', 'landing_page_view', 'location_view', 'parental_status_view', 'shopping_performance_view', 'ad_group', 'keyword'], status: 'captured', confidence: V, note: 'vendor serves conv-action at ad_group + keyword too; writer is campaign-only → SLICE (queue G-FILL#9). ⛔ THE SLICE IS UNCHANGED BY THE ENTITY AXIS: the grains after keyword are VENDOR-NAMED report resources the universe walk serves this segment from (LORAMER_UNIVERSE_ENTITY_AXIS_V1, 2026-08-03); ad_group and keyword are still genuinely MISSING and this stays a SLICE.' },
    impression_share: { grains: ['campaign', 'ad_group', 'keyword_view', 'customer', 'gender_view', 'shopping_performance_view'], status: 'captured', confidence: V, note: 'LORAMER_IMPRESSION_SHARE_FAMILY_V1 (2026-09-22): the walk captures the family at every resource the vendor serves it on (six, measured on Foam OH; search + content) for WALK connections; the legacy sync stays campaign/search. The vendor also lists it on hotel_*, product_group_view, shopping_product, targeting_expansion_view, travel_* — all observed zero on the probe account and emitted as family entries that will select when they deliver.' },
