...
const cursors = await q(`select client_id::text as client_id, platform, backfill_earliest_date::text as earliest, backfill_complete
   from sync_state where backfill_complete = true`)
const cursorByKey = new Map(cursors.map((r) => [`${r.client_id}|${r.platform}`, r]))

const claims = []
for (const conn of conns) {
  const defs = REQUIRED_STEPS[conn.platform]
  if (!defs) continue
  ...
  for (const sd of defs) {
    ...
    if (!cur) continue // no claim of completion → out of scope for THIS gate (the frozen-cursor detector owns that)
    ...
    claims.push({
      ...
      claimedFloor: cur.earliest, notRowCheckable: rp === null, rowsPresent: rp === true,
      minRowDate: matching.map((m) => m.mn).sort()[0] ?? null,
...
console.log(`  claims     : ${claims.length} completion claims across ${conns.length} connections`)
