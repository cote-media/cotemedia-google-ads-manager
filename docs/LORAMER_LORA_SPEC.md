...
Opus 4.8 is the MINIMUM model on ALL Lora surfaces — chat AND the insight/alert bar. The proactive engine (insight/alerts) is a HIGH-REASONING surface BY DESIGN: it must dig and find the real cause ("spend up because X search term fired 1000x and it's wrong for this business because…"), never emit generic "spend up 2%". Cost is not a constraint (real spend ~$10 / 6 weeks; Opus = 1.67x Sonnet, pennies/client). GUARDS: (a) fix the spend-logger MODEL_PRICING map to include current model rates BEFORE any switch (unmapped model logs $0 → cost reports silently lie); (b) keep prompt-caching ON (biggest cost lever). Prove Opus earns it via the eval A/B (Sonnet vs Opus on the golden set) before final commit.
