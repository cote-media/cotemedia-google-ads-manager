...
| impression_share  | google | campaign (legacy sync) · campaign, ad_group, keyword_view, customer, gender_view, shopping_performance_view (walk, LORAMER_IMPRESSION_SHARE_FAMILY_V1 2026-09-22) | 'search' (legacy) · 'search' / 'content' by metric prefix (walk) | intel.impressionShares (RIDES live-intel GAQL, no new call)      | ✅ LORAMER_GOOGLE_CONV_ACTION_IS_PERSIST_V1 (T0.2); forward-persist; 7 IS ratios in extra (count cols=0); WRITE-ONLY; campaign-only; history=T2.3 |
