...
| age                | campaign     | raw      | FROM age_range_view     | ⚠ CORRECTED 2026-07-15 — **FETCHED, NOT PERSISTED (G3 = an open DEFECT, not a capture)**. The old note "VERIFIED in-code (google-intelligence:392)" attested the **FETCH** and was read for three weeks as if it attested PERSISTENCE. It does not. google-intelligence.ts:414-437 fetches age_range_view + gender_view live on every run for the Lora prompt and DROPS the result: the only age/gender row-builder in the repo is meta-age-gender-backfill.ts, and a live skip-scan of metrics_daily returns **ZERO google age/gender rows, ever**. Per BEDROCK (4), fetched-but-unpersisted = DEFECT. Fix = persist-only (~free, the data is already in hand). |
| gender             | campaign     | raw      | FROM gender_view        | ⚠ CORRECTED 2026-07-15 — same as `age` above: FETCHED (google-intelligence.ts:414-437), NEVER PERSISTED, zero landed rows. G3. |
