...
- Conversion segmentation: segments.conversion_action, .conversion_action_name, .conversion_action_category, .conversion_lag_bucket, .conversion_or_adjustment_lag_bucket, .conversion_attribution_event_type, .external_conversion_source  — CAPTURED-FORWARD (T0.1, LORAMER_GOOGLE_CONV_ACTION_IS_PERSIST_V1, 2026-06-29): conversion_action_name→breakdown_value, conversion_action_category→extra (RAW ENUM, not name-mapped yet), per-action conversions+value persisted at campaign grain (breakdown_type='conversion_action'). FORWARD-ONLY (no history; 37-mo backfill = T2.3, quota-gated). Still dropped: lag_bucket / attribution_event_type / external_conversion_source. Pairs with the all_conversions writer in 5.4.
