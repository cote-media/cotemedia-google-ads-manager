...
### 5b-SETTLED · ⛔ PROBED 2026-08-15 — THE SPLIT RESOLVES, AND IT RESOLVES BOTH WAYS

| FACT | CLASS | SCOPE | SOURCE | ESTABLISHED | NOTES |
|---|---|---|---|---|---|
| PMax per-asset PERFORMANCE LABELS are NOT API-selectable on `asset_group_asset` in the API major we speak (v23) — the field does not exist | RESEARCHED-BY-MEASUREMENT | PER-ACCOUNT (Foam OH 7688521852) + VERSION-SCOPED (google-ads-api 23.0.0, pinned `^23.0.0` in package.json) | Probe 2026-08-15, 5 GAQL requests of a 10 cap. `SELECT asset_group_asset.asset, asset_group_asset.performance_label FROM asset_group_asset` → **`{"error_code":{"query_error":32},"message":"Unrecognized field in the query: 'asset_group_asset.performance_label'."}`** — a FIELD-EXISTENCE refusal, not an empty result. Foam OH genuinely has PMax: `asset_group` returned 20 groups on channel type 10 (PERFORMANCE_MAX), so the refusal is not an absent-inventory artifact. | 2026-08-15 | ⇒ **THE FIRST HALF OF OUR BANKED COMMENT HOLDS for PMax on v23.** The cited Google page is v24-era; whether v24+ adds the field is UNESTABLISHED and must not be asserted either way. ⚠ SCOPE: one account, one API major — the field's ABSENCE is a schema fact and therefore account-independent, but it is version-dependent and re-probing is the only thing that settles a version bump. |
| PMax per-asset METRICS **ARE** served on `asset_group_asset` — our banked "per-asset raw metrics also not exposed" is FALSE | RESEARCHED-BY-MEASUREMENT | PER-ACCOUNT (Foam OH) + VERSION-SCOPED (v23) | Same probe: `SELECT asset_group_asset.asset, asset_group_asset.field_type, asset_group.id, metrics.impressions, metrics.clicks, metrics.conversions FROM asset_group_asset WHERE segments.date DURING LAST_30_DAYS` → **20 rows, no error**, e.g. `asset=customers/7688521852/assets/31473724729 impr=0 clicks=0 conv=0`. The query is ACCEPTED; the zeros are this account's dormant PMax assets, not a capability wall. | 2026-08-15 | Google's own page (last updated 2026-07-22) says per-asset stats are "now available" — CONFIRMED for v23 by execution. Correct the comment at `build-claude-context.ts:764` when that surface is next touched; not edited in the probe flight. |
| `performance_label` IS served for RESPONSIVE SEARCH AD assets on `ad_group_ad_asset_view`, and we already read it | RESEARCHED-BY-MEASUREMENT | PER-ACCOUNT (Foam OH) + VERSION-SCOPED (v23) | Same probe, control leg: 30 rows returned, histogram `{"7":30}` — enum 7 = **NOT_APPLICABLE** (`AssetPerformanceLabel`, decoded from the installed library, not recalled). Read site: `google-intelligence.ts:606`. | 2026-08-15 | ⇒ **A BLANKET "the API does not expose performance labels" IS FALSE** — the field exists and is populated-shaped on the RSA path. ⚠ ON THIS ACCOUNT every RSA asset reads NOT_APPLICABLE, so we have NOT observed a real BEST/GOOD/LOW value anywhere; that a live label renders correctly end-to-end is UNESTABLISHED. |
