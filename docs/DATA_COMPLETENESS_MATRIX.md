...
| **Shopify** | base: account + product (NET); dimensional: geo_country, geo_region. Customer-mix in account extra. Verified live this session. | shopify-dimensional — RUN+PROVEN (Influential Drones + Escential + Foam OH, 2026-06-12) at the time truncated to ~60 days by the scope wall (now LIFTED on re-auth — see Known limits). | 2026-03-14 → 2026-06-11; 6 clients; 519 rows; account/product + geo_country/geo_region (pre-read_all_orders span) | **60-DAY SCOPE WALL — RESOLVED for capture (2026-06-17, LORAMER_SHOPIFY_READ_ALL_ORDERS_SCOPE_V1):** read_all_orders added to the OAuth routes + shopify.app.toml + `shopify app deploy` (loramer-7) → after re-auth the token carries read_all_orders and reaches the FULL order history. PROVEN on Foam OH: oldest order 2022-01-24 (#FOAMOH1001) now visible (vs the old ~2026-04 wall). Full Shopify history is now BACKFILL-ABLE — pending the backfill run. ⚠️ Foam OH re-authed only; Influential Drones + The Escential Group still hold the old 3-scope token (re-auth pending). Sell-through (read_inventory) + ATC funnel (Web Pixels) still not captured. |
