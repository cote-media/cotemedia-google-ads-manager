...
- [DATA] ⛔ The legacy dashboard's Shopify chart counted cancelled orders, so it showed more orders than Lora, a LOWER average order value, and a slightly HIGHER revenue for the same client · /dashboard · **CORRECTNESS** · AUDIT_FINDINGS:47 #6b — FIXED 2026-08-23 (LORAMER_SHOPIFY_CANCELLED_EXCLUDED_V1, guard-held). THIRD AND MEASURED STATEMENT: count, AOV, **and a small revenue over-report** — the earlier "revenue agrees, a cancelled order's subtotal is $0" was generalised from 7 orders and is FALSE; across 162 cancelled orders on two stores, 160 are $0 and TWO are not ($325.00 and $316.00). Corrected chart now equals the captured path to the cent (Foam OH 2025-02-01→03-31: both $46,412.30; unfiltered read $46,728.30).
