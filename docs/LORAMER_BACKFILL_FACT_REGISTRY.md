...
| Basic Access is 15,000 operations/day | RESEARCHED | GLOBAL | https://developers.google.com/google-ads/api/docs/api-policy/access-levels | 2026-08-10 | Verbatim: "15,000 operations / day for both test and production accounts". | ⟨history — Basic no longer applies: Standard Access GRANTED 2026-09-15; LORAMER_GOOGLE_ACCESS_STANDARD_V1 owns the status⟩
...
| ⛔ `universe-start` publishes to the **V1 topic** — there is NO operator path to the v2 topic | RESEARCHED-BY-MEASUREMENT | GLOBAL | runtime log 2026-08-12 18:16Z: a universe-start publish was consumed by `POST /api/queues/google-ads-universe` (v1); repo grep — the only v2-topic (`send(TOPIC` on `universe-v2-contract`) publishers are `cron/universe-resume` and the v2 consumer's own `advance()` | 2026-08-12 | **THE MEASURED INSTANCE WAS A 1-OP MISFIRE:** one bounded message on attested ground, meant to trigger the v2 consumer's first-touch inception discovery, was consumed by v1 — which has no discovery and does not read v2 attestations — spending 1 real request re-asking known-empty ground (`universe_window_log` 18:16:46Z, outcome zero). An operator needing a v2 consumption has exactly ONE lever: the resumer's owed-window computation (publish 0 when the scanned horizon is attested — QUEUE ★RESUMER-SCAN-CAP-NEVER-ROTATES). Verify the TOPIC of any publisher before firing it. |
...
| **Daily AND hourly grain served at 48 months, daily at 125 months — 109 days after the policy's effective date** | RESEARCHED-BY-MEASUREMENT | PER-ACCOUNT | 2026-09-18 round-14 probe, 14 ops on Tri-Copy 8289851425: campaign daily 2022-09-10..16 → 7 rows; campaign HOURLY same week → 144 rows; campaign daily 2016-04-20..26 (inception week) → 7 rows, 34,347 impressions; search terms at 125 months → 14 rows; segments.month 2016-04..06 → 3 rows | 2026-09-18 | Not one DateRangeError at any depth. Reconfirms the 08-10 rows with ROWS, not just success-empty, past the wall. The canary (LORAMER_RETENTION_WALL_CANARY_V1) re-asks the 2022-09 week daily. |
| click_view refuses beyond 90 days with a NAMED error | RESEARCHED-BY-MEASUREMENT | GLOBAL | probe K2: `SELECT click_view.gclid, segments.date FROM click_view WHERE segments.date = '2025-10-20'` (332 days back) | 2026-09-18 | REJECTED verbatim: `{"click_view_error":3} "The requested date is too old. It cannot be older than 90 days."` Matches the resource page ("must have a filter limiting the results to one day and can be requested for dates back to 90 days"). click_view delivers no metric and is not walked (catalog delivers:false). |
| The account-level activity query is legal with 9 metrics and NOT with metrics.video_views | RESEARCHED-BY-MEASUREMENT | GLOBAL | `SELECT segments.date, metrics.cost_micros, impressions, clicks, conversions, conversions_value, all_conversions, interactions, view_through_conversions, engagements FROM customer WHERE segments.date BETWEEN '2025-10-10' AND '2025-10-25'` → 12 rows (2025-10-10..21); with metrics.video_views → `{"query_error":32} Unrecognized field` | 2026-09-18 | The idle skip's query shape (universe-idle-skip.ts ACTIVITY_METRICS). Days the vendor omits are days on which every selected metric was zero (zero-metrics rule). |
| Round 13's 840 zero descend requests on Tri-Copy were an IDLE YEAR, not a wall | VERIFIED | PER-ACCOUNT | universe_attempt_log 2026-09-17 23:13–00:27Z: descend 840 attempts 2025-11-10..2026-01-11 all 'zero'; missed lane 2016 windows 26 ok / 763 rows; metrics_daily account rows: last spend day 2025-10-21, 2026 = 107 days at $0 | 2026-09-18 | The emptiness begins where spending stopped, 10–11 months back — inside any wall. LORAMER_IDLE_SKIP_V1 retires such windows for one request. |
...
| `covered` and `attestedEmpty` PARTITION the window — a day with rows is COVERED and the attestation YIELDS to it | TESTED | GLOBAL | `windowCoverage` filters `attestedEmptyDays` disjoint from `covered` (universe-coverage.ts, LORAMER_COVERAGE_SETS_PARTITION_V1); `universe-stream-consumer.guard.mjs` leg (b1a) drives the REAL compiled `windowCoverage` through a chainable stub where every day holds rows AND a zero spans the window — proven RED first (2 findings against the pre-fix file: overlap [01,02,03]; 3+4+0=7 over a 4-day window) | 2026-08-12 | Found by the FIRST fire the base aliases were live for (01:30Z): ad_group + ad_group_ad base days were covered via forward's `''` rows (the alias) AND attested by the previous night's paid zeros; `assessCoverage` summed 28+30+0=58 over a 30-day window and refused BOTH surfaces — durably, for zero requests. Before the aliases the sets were disjoint by ACCIDENT; nothing enforced it. The gate's invariant is right — the sets now satisfy it by construction. Red-proof note: the guard copy must live in its OWN directory — Node 20 memoises (parent.path, request) above the `_resolveFilename` hook, so a same-directory copy silently re-uses the original's throwing-stub resolution. |

## AD.NAME IS VENDOR-EMPTY FOR SEARCH-TYPE ADS — PROBED 2026-08-15 (★GOOGLE-AD-ENTITY-NAMES-MISSING diagnosis)

| FACT | CLASS | SCOPE | SOURCE | ESTABLISHED | NOTES |
|---|---|---|---|---|---|
| Google serves NO `ad_group_ad.ad.name` for search-type ads (RSA, expanded text, DSA and 2 further unnamed types) — the field is ABSENT from the response, not empty | RESEARCHED-BY-MEASUREMENT | PER-ACCOUNT (Foam OH, 7688521852; matches the vendor's documented field support) | Probe 2026-08-15, 6 GAQL requests of a 20 cap: `SELECT ad_group_ad.ad.id, ad_group_ad.ad.type, ad_group_ad.ad.name FROM ad_group_ad` → by type: 7 (10 ads) 0 named · 9 (2) 0 · 10 (4) 0 · 15=RESPONSIVE_SEARCH_AD, proven by a type-filtered cross-probe (22) 0 · 19 (6) 0 — 44 of 50 ads carry NO name; raw `name` reads `undefined`. Vendor doc RE-CONFIRMED against the LIVE proto (github.com/googleapis/googleapis, google/ads/googleads/v22/resources/ad.proto, field 47, fetched 2026-08-15): "The name field is currently only supported for DisplayUploadAd, ImageAd, LegacyAppInstallAd, ShoppingComparisonListingAd, VideoAd, VideoResponsiveAd and DemandGen ads." (WIDER than the reference-page paraphrase first banked — three more supported types; none of them search-type, so the diagnosis stands unchanged.) | 2026-08-15 | The 6 ads that ARE named (types 30/40/42) carry AUTO-GENERATED junk — verbatim `"Ad #1"`, `"Ad 1"`, `"(Ad 1) auto-generated video ad"` — so even the named minority is not a human-meaningful name. |
| The name-bearing fields ARE served for RSAs: `responsive_search_ad.headlines[].text` / `.descriptions[].text` come back populated on the same ads whose `name` is absent | RESEARCHED-BY-MEASUREMENT | PER-ACCOUNT (Foam OH) | Same probe, request 2: ad 590996928877 `name=undefined`, `headlines[0..3]=["Don't FOMO, Foam-Oh","More Pieces than any other","ONLY sofa with personalization","Free Shipping. Made in the USA"]` (two siblings equally populated). `/api/google/ads/route.ts:70-78` already composes display names from exactly these fields on the live path. | 2026-08-15 | ⇒ the warehouse's 1.7%-named ad grain is the VENDOR'S answer faithfully stored (`google-adgroup-ad-backfill.ts:139-140` maps `ad.name \|\| ''`); the fix is compose-from-headlines at the WRITER, not a capture bug hunt. QUEUE ★GOOGLE-AD-ENTITY-NAMES-MISSING owns status. |
