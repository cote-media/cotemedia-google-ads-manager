...
BUILD TARGET (gate-level): -next is the PRIMARY UI surface for go-forward work. The live app is NO LONGER FROZEN (Meta App Review APPROVED 2026-07-02) — live-path work is permitted with graduated care. Every proposed step states its BLAST RADIUS: read-only (safe) → backend writer (isolated) → -next UI (preview-gated) → live-path (shared read-path / reviewer surfaces). A live-path step is a STOP-and-confirm, never automatic — not because of a freeze, but because of blast radius + the reviewer-path COMPLIANCE holds (Meta ~1yr reviewer creds, Shopify review still open, Meta data-deletion callback).
