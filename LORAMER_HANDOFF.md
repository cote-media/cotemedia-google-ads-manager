...
WEB-FIRST DIAGNOSIS — REPORT REQUIREMENT (LORAMER_WEB_FIRST_DIAGNOSIS_V1, 2026-07-19, mandatory). The law lives in ESSENCE; this is the flight-level expectation that makes skipping it VISIBLE. **A diagnosis report is INCOMPLETE — not merely thin, INCOMPLETE — unless it states, as its own named section, the result of a web search for (a) the KNOWN CAUSE of the error and (b) an EXISTING PUBLISHED FIX or implementation.** The section is required even when the search finds nothing: "searched <what>, no known cause / no existing fix found" is a complete answer; SILENCE IS NOT. Its absence means the search was skipped, and the report should be sent back for it. State plainly whether an existing base was ADAPTED or the fix was authored from scratch, and why. Adapted code carries the SAME Gate-A burden as authored code — read in full, proven live, never trusted because it came from a repo with stars. WHY THIS IS A REPORT RULE AND NOT JUST A HABIT: on 2026-07-19 the Meta 1/99 fix was engineered end-to-end before any search; the search afterwards confirmed the error is documented as a payload-size 500 with ID-batching as the standard remedy, and turned up meta-ads-mcp already handling Meta creative errors. Nothing in the report showed the search was missing, because there was no place it was supposed to be. Now there is.
