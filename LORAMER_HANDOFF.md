...
FAST-PATH RESUME (default; LORAMER_RESUME_DIGEST_V1) — a SINGLE-PASTE digest IN FRONT of the full tiered read above, NEVER a replacement:
- "resume loramer" may instead output the SHORT paste: `cd <repo-for-this-machine> && git pull origin main && git rev-parse HEAD && cat docs/HANDOFF_MANIFEST.json && cat LORAMER_RESUME_DIGEST.md`.
- FRESHNESS GATE (the safeguard against the stale-doc lie that cost three days): LORAMER_RESUME_DIGEST.md is REGENERATED from source by `scripts/build-resume-digest.mjs` (NEVER hand-edited) and carries a per-source-doc content_hash stamp in its section A. The digest is CURRENT iff EVERY source-doc content_hash in it MATCHES the live docs/HANDOFF_MANIFEST.json. ALL MATCH → the digest is current → read it IN FULL, restate the relevant decisions/queue per RESTATE-TO-PROVE (4c), state NEXT STEP, WAIT for "go" — ONE paste, done. ANY MISMATCH, or the digest missing → it is STALE → IGNORE it and FALL BACK to the full tiered read above (steps 1-5). The digest is a DERIVED fast path; on any doubt the authoritative docs + full read win. (Gate on the section-A content_hashes — drift-exact; the digest's built_from-HEAD is informational only, since unrelated commits change HEAD without changing the digest's sources.)
...
Every platform, every grain, as deep as the source allows — and every gap surfaced EXPLICITLY, never a silent empty. A missing platform, a truncated window, a dropped grain is a BUG until proven to be a documented limit; any accepted cap (retention floor, scope wall, API/row cap) is a LOGGED, deliberate decision recorded in **docs/DATA_COMPLETENESS_MATRIX.md** (the scorecard) — which is updated in the SAME commit as any change to what a platform captures. First-principle statement lives in LORAMER_ESSENCE.md. This is why a false-zero or a scope-truncated backfill is treated as a data-integrity defect, not an acceptable approximation.
...
## STANDING — -next is the ONLY development target going forward
All forward development happens in the -next environment. The live (old) app is no longer a development target — we don't invest in it, improve it, or chase parity-with-live. Its reviewer surfaces stay FROZEN / untouched until the Meta App Review decision lands (external constraint — touching them mid-review could affect the outcome); leaving the live app entirely alone satisfies both "don't worry about the live version" and the freeze. When -next reaches parity and the approvals land, -next flips to default and the old app is retired.
