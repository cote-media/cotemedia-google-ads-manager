...
6c. FRESHNESS-GATE HARD CHECK (LORAMER_STALE_CONTENT_CLEANUP_V1) — a GATE, NOT a suggestion. After steps 6 + 6b, run the freshness gate over the EIGHT gated SOURCE_DOCS (the 5 registers + docs/LORAMER_DEFINITIVE_CAPTURE_INVENTORY.md + docs/LORAMER_BREAKDOWN_REGISTRY.md + RESUME_INSTRUCTIONS.md — the SOURCE_DOCS list in scripts/build-resume-digest.mjs): for EACH, live SHA-256 == the digest §A stamp == the manifest content_hash. It MUST read 8/8 PASS. A skipped re-stamp on ANY changed gated doc = a RED gate on the next resume = STOP — do NOT commit on less than 8/8. (Root cause this kills: the 2026-06-29 staleness audit found load-bearing docs unguarded because "in the manifest" did not mean "gate-checked"; widening SOURCE_DOCS 5→8 + this check makes the two equivalent — manifest membership now MEANS guarded.)
