...
## ⛔ STANDING RULES BANKED 2026-08-16 (gate-level; read every session)
<!-- LORAMER_STANDING_RULES_2026_08_16_V1 -->
Four rules from the 2026-08-15/16 close. Each states its own enforceability per `LORAMER_A_LAW_IS_NOT_BANKED_UNTIL_IT_CAN_FAIL_A_BUILD_V1` — none of these is pretending to be a guard.

- **MODEL ROUTING — every paste states which model it needs.** `FABLE-NEEDED` (design judgment, novel diagnosis, adversarial reasoning about an unproven mechanism) or `MECHANICAL` (script execution, docs, verification against an already-proven Gate-A). **The Fable weekly quota is metered — route around it exactly like vendor quota, and put it on the same line as cost.** A mechanical paste burning Fable quota is the same waste as a backfill burning the forward-sync reserve. *Enforceability: UNENFORCEABLE — it governs what the paste says, and pastes are not in the repo.*
- **SOLE-USER REALITY — Russ is the ONLY user of Lora, and defect urgency is computed against that.** "1.77% of turns" is not customer impact when there are no customers. ⛔ **DO NOT CALL A DEFECT CUSTOMER-FACING WITHOUT CHECKING WHO ACTUALLY HIT IT** — the check is cheap (persistence in `client_conversations` separates harness traffic from browser traffic; `user_email` does NOT, because the eval harness authenticates as Russ). The instance is [[LORAMER_TRUNCATION_BLAST_RADIUS_V1]]: nine truncations, eight to a test harness, one to a human, across three and a half weeks. *Enforceability: UNENFORCEABLE — it governs a judgement about severity. The nearest mechanical aid is the split query itself, which is written out in that entry and is reusable.*
- **RUN-IT-FOR-REAL — no check, guard or meter ships without ONE successful run against the real target inside Gate-A, output quoted.** Not compiled, not reasoned about, not "it should work": run, and paste what it printed. Earned repeatedly: the fleet-meter guard's first draft used the very PostgREST aggregate it existed to ban and only running it revealed that; the ad-name repair dry-run reported a 1,000-row page cap as a total; a `check-lora-named-entity` green was a 57014 timeout read as "no rows". *Enforceability: PARTIAL — `scripts/run-guards.mjs` already separates FAILED from CRASHED so a guard that cannot run is not counted as evidence, which catches the crudest version. It cannot catch a guard that runs against a stub and proves nothing.*
- **BLOCKING RULE — when work is in flight, state whether it BLOCKS the next paste.** Passive work (watchers, pollers, background confirmations, a deploy building) NEVER blocks — hand Russ the next paste immediately and say what is running. One-paste-in-flight (see CADENCE above, and the IN-FLIGHT GATE in CLAUDE.md) applies ONLY to actionable flights. Sitting idle beside a poller is a wasted round trip on the scarcest resource there is. *Enforceability: UNENFORCEABLE — same reason as MODEL ROUTING.*

Cross-refs, not copies: the SEAMS GATE lives in `CLAUDE.md` (repeat-offense rules bind the executor, per the RULE-HOME LAW) with `LORAMER_SEAMS_PROOF_V1` in DECISIONS owning the reasoning. TRUNCATED-COST-IS-A-FLOOR is owned by QUEUE ★E10-EXHAUSTS-THE-CAP where its measurement lives.

## ⛔ BUILD TARGET (gate-level; read every session)
<!-- LORAMER_HANDOFF_BUILD_TARGET_V1 -->
BUILD TARGET (gate-level): -next is the PRIMARY UI surface for go-forward work. Live-path work is permitted with graduated care — no Meta review, no reviewer path, no freeze (the Meta App Review outcome is owned by DECISIONS). Every proposed step states its BLAST RADIUS: read-only (safe) → backend writer (isolated) → -next UI (preview-gated) → live-path (shared read-path / any live surface). A live-path step is a STOP-and-confirm because of blast radius (every client, every live surface), never a reviewer. Standing PRODUCTION obligations (not a freeze): Meta data-deletion/deauth callback stays live, Shopify install callback is a LIVE merchant path provisioning must not break, Google Ads API access STANDARD since 2026-09-15, no daily cap (DECISIONS LORAMER_GOOGLE_ACCESS_STANDARD_V1 owns the status).
...
- **DESKTOP=MOBILE COHERENCE — AMENDED 2026-08-16 (Russ): DESKTOP AUTHORS, MOBILE VIEWS.** The redesign is ONE app and it is never forked. What must be IDENTICAL on both surfaces: the LAYOUT the user built, the DATA, the CONTENT, every card kind and every metric — readable, legible and coherent on both. What MAY differ: **AUTHORING capability only** — customize/edit mode, add card, remove card, resize, drag-reorder and per-card chrome may be DESKTOP-ONLY. Mobile DISPLAYS the layout built on desktop; it does not build one. PRECEDENT, cited because we follow it rather than invent it — Shopify admin Analytics: "You can customize your dashboard only by using your Shopify admin on a desktop computer. However, changes that you make to your dashboard display on both desktop and mobile devices." (help.shopify.com/en/manual/reports-and-analytics/shopify-reports/overview-dashboard/customizing-overview-dashboard) ⛔ THIS IS NOT A LICENCE TO SHIP LESS TO THE PHONE. A capability that is READING is not authoring: if a number, a card, a chart, a filter or an answer is reachable on desktop, it is reachable on mobile. Verify each increment on both surfaces before calling it done. | LORAMER_DESKTOP_AUTHORS_MOBILE_VIEWS_V1
- **EVERYTHING IS LAUNCH-CRITICAL ("Russ launch-blocker"):** default-treat all features/fixes as launch-critical for the soft launch (the launch DATE is owned by DECISIONS: LORAMER_SOFT_LAUNCH_JULY22_V1 — never restated here; Russ wants ~100% operational and will put in the hours). The ONLY exceptions are (a) true next-phase capabilities — design (Canva-like integration) then write capability; and (b) genuine external constraints (approval timelines — e.g. Google Ads Standard Access, whose live status is owned by DECISIONS/QUEUE). Never assume an item is post-launch unless it is one of those, or Russ says so.
...
WEB-FIRST DIAGNOSIS — REPORT REQUIREMENT (LORAMER_WEB_FIRST_DIAGNOSIS_V1, 2026-07-19, mandatory). The law lives in ESSENCE; this is the flight-level expectation that makes skipping it VISIBLE. **WIDENED 2026-07-23: this applies to ANY build / fix / detector / schema change / diagnosis paste, not only error diagnosis. Every such paste from Claude MUST carry a VISIBLE THREE-FIELD HEADER above the code block — HISTORY (already decided or built? cite it, don't reopen), WEB (the known cause + an existing published fix, or "searched X, none found"), REPO (verified against the current file) — each a one-line result OR a stated reason for n/a. A paste WITHOUT the header is MALFORMED — not thin, malformed — and Russ rejects it on sight, without reading the content.** The section is required even when the search finds nothing: "searched <what>, no known cause / no existing fix found" is a complete answer; SILENCE IS NOT. Its absence means the search was skipped, and the report should be sent back for it. State plainly whether an existing base was ADAPTED or the fix was authored from scratch, and why. Adapted code carries the SAME Gate-A burden as authored code — read in full, proven live, never trusted because it came from a repo with stars. WHY THIS IS A REPORT RULE AND NOT JUST A HABIT: on 2026-07-19 the Meta 1/99 fix was engineered end-to-end before any search; the search afterwards confirmed the error is documented as a payload-size 500 with ID-batching as the standard remedy, and turned up meta-ads-mcp already handling Meta creative errors. Nothing in the report showed the search was missing, because there was no place it was supposed to be. Now there is. CODE-SIDE HALF (Claude Code): every Claude Code diagnosis/build report must include a KNOWN-EXTERNAL-CAUSE line — what was searched, what was found, or why not applicable. That line's PRESENCE is mechanically inspectable — a report shows it or it does not. The Claude-side half is NOT inspectable: no guard can see whether a search actually happened BEFORE the code existed — the temporal order of search-vs-authorship is invisible to any check. So this is enforced at the point of DELIVERY (a headerless paste is rejected), never at the point of authorship; do not mistake the header for full enforcement. THE LOOPHOLE THAT DROVE THE WIDENING: T0#4 was a detector BUILD with no error, so the old "diagnosis report" framing did not fire while the spirit did — the header now fires on any build/fix/detector/schema/diagnosis, per the ESSENCE law.
...
- CAPABLE locally as of 2026-07-01, ⚠ **GA CLAUSE CORRECTED 2026-07-31**: Google Ads (GAQL), Supabase, Anthropic, **live Shopify (read + app-path mint)**, cron auth (`CRON_SECRET` real), **and live GA — the "NOT capable: live GA (creds blank)" clause that stood here was falsified by DECISIONS LORAMER_GA_CREDS_RESOLVED_V1 (2026-07-24), which proved a live GA4 runReport from this machine.** NOT capable: Stripe webhook/portal. ⛔ Per-variable state is not restated here — cite the decision and measure lengths.
...
- ⚠ **VERCEL CLI TOKEN IS INVALID ON THE AIR — verified 2026-08-02.** Any `vercel` command returns `{"error":{"code":"forbidden","message":"Not authorized","invalidToken":true}}` at http 403. **NOT BLOCKING, and the workaround is the point of recording this:** deploys are confirmable through the **Vercel MCP** via `list_deployments` with `projectId` + `teamId` (returns `state`, `githubCommitSha`, `target`) — that path worked in the same session the CLI failed. ⛔ `get_deployment` with a deployment URL returned **404** and is NOT the fallback; use `list_deployments`. Needs `vercel login` eventually — a HUMAN step (interactive browser auth), so it cannot be done by Claude Code. It also means **Lesson 48's env-write path (`vercel env add/rm`) is unavailable from the Air until re-auth** — that is the one thing this actually costs, since the MCP has no env-write tool.
...
## Current state — OWNED ELSEWHERE, not snapshotted here

The live current-state snapshot that used to live here (dated 2026-06-03) was DELETED 2026-07-17 — a six-week-stale copy is a lie with a timestamp (DECISIONS: NEVER WRITE DOWN A FACT ANOTHER DOC OWNS). Current state is owned by CONTINUE_HERE (the resume point + session log) and DECISIONS (settled status). Do not re-snapshot it here.
...
6c. FRESHNESS-GATE HARD CHECK (LORAMER_STALE_CONTENT_CLEANUP_V1) — a GATE, NOT a suggestion. After steps 6 + 6b, run the freshness gate over the NINE gated SOURCE_DOCS (the 5 registers + docs/LORAMER_BREAKDOWN_REGISTRY.md + RESUME_INSTRUCTIONS.md + docs/LORAMER_ASSET_LAYER_SCOPE_V1.md + docs/LORAMER_SECURITY_POSTURE.md — the SOURCE_DOCS list in scripts/build-resume-digest.mjs; docs/LORAMER_DEFINITIVE_CAPTURE_INVENTORY.md was RETIRED from the gated set 2026-07-17, LORAMER_DOCS_SINGLE_OWNER_V1): for EACH, live SHA-256 == the digest §A stamp == the manifest content_hash. It MUST read 9/9 PASS. A skipped re-stamp on ANY changed gated doc = a RED gate on the next resume = STOP — do NOT commit on less than 9/9. (Root cause this kills: the 2026-06-29 staleness audit found load-bearing docs unguarded because "in the manifest" did not mean "gate-checked"; widening SOURCE_DOCS 5→8→10 + this check makes the two equivalent — manifest membership now MEANS guarded.)
...
⛔ **AMENDED 2026-08-04 — LESSON 42 IS SATISFIED, NOT REPEALED, AND IT WAS NARROWER THAN IT WAS EVER RESTATED.** Two separate corrections, and the second is the more important:

**(1) THE CONDITION ENDED.** The lesson is conditional on an OPEN verification. Per Russ (2026-08-04, from chat history — not re-derived here): the 2026-07-14 Verification Center showed **Branding verified · Data access verified · `.../auth/adwords` approved**, and on **2026-07-17** Google sent *"Your app's data access has been verified"* at APP level, covering BOTH `adwords` and `analytics.readonly`, with no pending items. ⛔ **THAT IS TEN DAYS BEFORE LESSON 42 WAS WRITTEN ON 2026-07-27**, in a session that still described adwords as "under review". The lesson was authored against a condition that had already ended.

**(2) A REDIRECT URI WAS NEVER ON THE LIST.** Read the lesson's own enumeration: *app name, authorized domains, scopes, homepage/privacy/TOS URLs, or logo* — all **CONSENT SCREEN** fields. **Authorized redirect URIs are configuration of the OAuth CLIENT, a different object, and they appear nowhere in Google's warning or in this lesson.** So even while the review WAS open, this lesson never prohibited adding one. The prohibition was invented in the restatement, not inherited from the source.

⛔ **AND THE RESTATEMENT GOT STRONGER EACH TIME IT WAS REPEATED, ending at "adding preview domains revokes GA refresh tokens" — a claim Google's documentation does not support.** Token invalidation comes from revocation, scope changes, and the 100-tokens-per-account limit; adding a redirect URI is on none of those lists. **That sentence, not the lesson, is what blocked the chat-UI arc for over a week.** See DECISIONS LORAMER_CONDITIONAL_RULE_LOST_ITS_CONDITION_V1.
...
Forward development happens primarily in the -next environment; we don't invest in the live (old) app for its own sake or chase parity-with-live. There is NO Meta review and no reviewer surface — the Meta App Review outcome that retired it is owned by DECISIONS. What persists is COMPLIANCE with PRODUCTION obligations, not a freeze and not a reviewer: the Meta data-deletion/deauth callback must stay live (permanent Meta prod requirement), the Shopify App Store install callback is a LIVE merchant path provisioning must not break, and Google Ads API access is STANDARD since 2026-09-15 with no daily operations cap (DECISIONS LORAMER_GOOGLE_ACCESS_STANDARD_V1 owns the status; only the vendor's per-second QPS buckets bind route decisions) — so those live surfaces are touched only with a stated live-path blast radius + STOP-and-confirm, never casually, because of blast radius across every client, never a reviewer. The cutover flip (-next → default, old app retired) is UNBLOCKED and gated on -next feature parity (Ask-Lora chat + drill pages) per the cutover audit (LORAMER_NEXT_TO_LIVE_AUDIT_V1).

## STANDING — mobile experience is a first-class standard for ALL -next surfaces
The LoraMer mobile experience on every -next surface must be as modern, responsive, intuitive, and beautiful as possible — mobile is NOT an afterthought or a shrunk-down desktop port. The bar = the Claude app's own mobile behavior: the keyboard rises smoothly and the input renders beautifully for typing; in Code specifically, when there's a command/action to approve, the content shifts up in the input area so the action and the text stay visible and reachable. Apply that caliber: input fields that adapt gracefully when the keyboard or an action/approval appears; smooth, native-feeling interactions; thumb-reachable controls; nothing cramped. This applies app-wide, with the Lora chat input as the exemplar. Desktop + mobile remain ONE responsive codebase (chrome adapts, content/IA never forks) — and the mobile rendering must be genuinely excellent, not merely functional. Treat mobile polish as acceptance criteria on -next UI work, not a later pass. ⛔ UNCHANGED BY LORAMER_DESKTOP_AUTHORS_MOBILE_VIEWS_V1 (2026-08-16): permitting DESKTOP-ONLY AUTHORING does not lower this bar by one point. Everything mobile still does — reading, scrolling, filtering, charting, Lora — is held to the same first-class standard, and 'mobile views' means the VIEW is excellent, not that the phone gets a cut-down port.
...
Commit and push in ONE motion when, and ONLY when, the change is verified clean:
  ...
  • Blast radius declared + honored (any live-path / shared-read-path change was STOP-and-confirmed; owner-gate count unchanged)
