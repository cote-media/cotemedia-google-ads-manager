...
BUILD TARGET (gate-level): -next is the PRIMARY UI surface for go-forward work. Live-path work is permitted with graduated care — no Meta review, no reviewer path, no freeze (Meta App Review APPROVED 2026-07-02). Every proposed step states its BLAST RADIUS: read-only (safe) → backend writer (isolated) → -next UI (preview-gated) → live-path (shared read-path / any live surface). A live-path step is a STOP-and-confirm because of blast radius (every client, every live surface), never a reviewer. Standing PRODUCTION obligations (not a freeze): Meta data-deletion/deauth callback stays live, Shopify install callback is a LIVE merchant path provisioning must not break, Google Ads Standard Access still pending (separate application).
...
Forward development happens primarily in the -next environment; we don't invest in the live (old) app for its own sake or chase parity-with-live. There is NO Meta review and no reviewer surface — Meta App Review was APPROVED (2026-07-02). What persists is COMPLIANCE with PRODUCTION obligations, not a freeze and not a reviewer: the Meta data-deletion/deauth callback must stay live (permanent Meta prod requirement), the Shopify App Store install callback is a LIVE merchant path provisioning must not break, and Google Ads Standard Access is still pending (a separate application; the 15k/day Basic cap binds until it clears) — so those live surfaces are touched only with a stated live-path blast radius + STOP-and-confirm, never casually, because of blast radius across every client, never a reviewer. The cutover flip (-next → default, old app retired) is UNBLOCKED and gated on -next feature parity (Ask-Lora chat + drill pages) per the cutover audit (LORAMER_NEXT_TO_LIVE_AUDIT_V1).
