...
// LORAMER_ONE_CLICK_RUN_V1 (flight 2, 2026-09-18): kickoffWalk — the one-turn resumer kick the -next button fired for
// google — is RETIRED. The button now starts the continuous run once (client-run-start.ts); the run's own steps are the
// fires, and a kick beside a run is a wasted lease. Deleted rather than left as a dead export: an export with a guard
// on it and no caller is a guard on nothing.
