...
export function buildClaudeContextCacheable(
  ...
  // LORAMER_LORA_COVERAGE_V1 (Fix #1 Part B) — ANSWER FROM THE COVERAGE FACT. query_metrics attaches coverage.state per
  // platform/window; Lora reads the STATE instead of guessing from ambiguous rowCount-0 zeros (identical for
  // not-connected / pre-capture / true-zero). Canonical rules apply ONLY in the 'covered' state.
  lines.push(`REVENUE / ROAS / METRIC ANSWERS — ANSWER FROM THE COVERAGE FACT. Each query_metrics window carries a coverage array (one entry per platform) with a state. Answer FROM that state — it is fact, not a guess, and it OUTRANKS the availability-fallback:
- not_connected — the platform is NOT connected for this client. Say "{platform} isn't connected." NEVER report zeros; do NOT call it "no data captured".
- predates_capture — the window is before our earliest captured data (coverage.captureFloor). Say "we have no captured {platform} data before {captureFloor}." NEVER report $0/zero as a measured figure; you may NOT claim the account had no activity — we cannot know that.
- draining_unknown — history is still importing. Say you CANNOT confirm a zero for that period yet; do NOT report $0 as real.
...
3 A FORWARD-ONLY FAMILY ON A LEGACY CONNECTION — Google conversion_action settings have no backfill (first row 2026-06-27). Impression share: on a LEGACY connection the sync writes campaign/search only, from 2026-06-27; on a WALK connection the walk captures the whole family (every resource the vendor serves it on, search and content) back to Google's 37-month wall — measured 2026-09-22: 1,005 campaign-days to 2023-08-22 on one account. Absence before 2026-06-27 on a legacy connection is a LoraMer fact; on a walk connection it is not.
