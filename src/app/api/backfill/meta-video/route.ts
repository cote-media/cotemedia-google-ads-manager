// LORAMER_META_VIDEO_ROUTE_V1
// CRON_SECRET-bearer GET wrapper over runMetaVideoBackfill — the thin targeted-invoke TWIN every Meta breadth
// writer has (mirrors /api/backfill/meta-action-type EXACTLY: same auth, same clientId/startDate/endDate/dryRun
// params, STATELESS bounded-range loop from endDate BACKWARD within a wall-clock budget; NO cursor / NO sync_state
// — the drain's rangeLap owns the 'meta_video' cursor). Freeze-safe backend, no UI. WINDOW_DAYS=90 matches the
// drain step (video = FIELD-WIDEN, only 4 reports/sub-range → a wide window is safe under the 300s route).
import { NextResponse } from 'next/server'
import { runMetaVideoBackfill } from '@/lib/backfill/meta-video-backfill'
...
export async function GET(request: Request) {
  ...
  const writer = runMetaVideoBackfill
  ...
  while (true) {
    ...
    subRanges.push({ range: body.range, written: body.written, levels: body.levels })
