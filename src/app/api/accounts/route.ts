...
async function gadsRequest(accessToken: string, customerId: string, body: any) {
...
export async function listAccessibleAccounts(refreshToken: string) {
  ...
  const res = await fetch(
    `https://googleads.googleapis.com/v17/customers/${MANAGER_ID}/googleAds:search`,
    {
      ...
      body: JSON.stringify({
        query: `SELECT customer_client.client_customer, customer_client.descriptive_name, customer_client.currency_code, customer_client.time_zone, customer_client.status FROM customer_client WHERE customer_client.level = 1 AND customer_client.status = 'ENABLED'`
      }),
    }
  )
  const data = await res.json()
...
export async function getCampaigns(refreshToken: string, customerId: string, dateRange = 'LAST_30_DAYS') {
  const accessToken = await getAccessToken(refreshToken)
  const results = await gadsRequest(accessToken, customerId, {
    query: `SELECT campaign.id, campaign.name, campaign.status, campaign.advertising_channel_type, campaign_budget.amount_micros, metrics.impressions, metrics.clicks, metrics.cost_micros, metrics.conversions, metrics.conversions_value, metrics.ctr, metrics.average_cpc FROM campaign WHERE segments.date DURING ${dateRange} AND campaign.status != 'REMOVED' ORDER BY metrics.cost_micros DESC`
  })
  return results.map((row: any) => ({
    id: row.campaign?.id,
    ...
    budget: row.campaignBudget ? (row.campaignBudget.amountMicros / 1_000_000).toFixed(2) : null,
    impressions: row.metrics?.impressions || 0,
    clicks: row.metrics?.clicks || 0,
    cost: ((row.metrics?.costMicros || 0) / 1_000_000).toFixed(2),
    conversions: row.metrics?.conversions || 0,
    conversionValue: row.metrics?.conversionsValue?.toFixed(2) || '0',
    roas: row.metrics?.conversionsValue && row.metrics?.costMicros > 0
      ? (row.metrics.conversionsValue / (row.metrics.costMicros / 1_000_000)).toFixed(2)
      : null,
    ctr: ((row.metrics?.ctr || 0) * 100).toFixed(2),
    avgCpc: row.metrics?.averageCpc ? (row.metrics.averageCpc / 1_000_000).toFixed(2) : null,
  }))
}
...
export async function getSearchTerms(refreshToken: string, customerId: string, dateRange = 'LAST_30_DAYS') {
  const accessToken = await getAccessToken(refreshToken)
  const results = await gadsRequest(accessToken, customerId, {
    query: `SELECT search_term_view.search_term, search_term_view.status, campaign.name, ad_group.name, metrics.impressions, metrics.clicks, metrics.cost_micros, metrics.conversions, metrics.ctr FROM search_term_view WHERE segments.date DURING ${dateRange} ORDER BY metrics.cost_micros DESC LIMIT 500`
  })
  return results.map((row: any) => ({
