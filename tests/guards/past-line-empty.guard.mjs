...
//   (b) PURE idleVerdict: an empty account answer past the line reads 'idle' exactly as above it (the round-42 hold lifted by LORAMER_IDLE_SEED_RETRACTION_V1)
...
if (I) {
  for (const c of ['served', 'unknown']) {
    ...
    check(v.kind === 'idle', `(b) ⛔ past the line an empty account answer read '${v.kind}' under canary '${c}' — LORAMER_IDLE_SEED_RETRACTION_V1: the probe answers past the line (measured); the account's own answer retires the window past the line exactly as above it.`)
  }
