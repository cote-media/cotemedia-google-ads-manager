#!/usr/bin/env node
// LORAMER_GOOGLE_UNIVERSE_WRITER_V1 — GUARD. THREE LEGS.
//
...
const findings = []
const fail = (m) => { console.error(`[google-universe-writer] FAIL — ${m}`); process.exit(1) }
const read = (p) => { try { return readFileSync(resolve(ROOT, p), 'utf8') } catch { return null } }

const SRC = 'src/lib/backfill/google-universe-writer.ts'
const ART = 'docs/google-capture-universe.json'
...
let mod
try { mod = req(join(out, 'src/lib/backfill/google-universe-writer.js')) }
...
if (findings.length) {
  console.error(`[google-universe-writer] FAIL — ${findings.length} finding(s):`)
  for (const f of findings) console.error(`  - ${f}`)
  process.exit(1)
}
console.log(`[google-universe-writer] PASS — no clock is reachable (no floor36, no Date, no month arithmetic; completion driven only by a vendor zero and carrying its proof); the writer names ZERO of the artifact's surfaces and contains no switch; an unsatisfiable structural requirement is recorded before a request is spent, a satisfiable one still runs, and a vendor zero is recorded as a fact. ⛔ NOT ASSERTED: that Google accepts the GAQL or that the artifact's delivers values are still true — those need the vendor.`)
